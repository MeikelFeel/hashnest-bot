...
class hashnest(object):
    ...
    def signature(self,req):
        ...
        req['nonce']=nonce
        req['signature']= hmac.new(self.secret.encode('utf-8'), msg=message.encode('utf-8'), digestmod=hashlib.sha256).hexdigest()
        return urlencode(req).encode('utf-8')

    def request(self,url,req={}):
        url = self.URL + url
        data= self.signature(req)
        info = urllib.request.Request(url, data)
